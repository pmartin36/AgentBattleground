...
impl BattleMenu {
    ...
    /// Inset of the close button from the panel's top and right. Top half of the
    /// circle is removed, so a 0-cell top inset lands the "X" 4 dots below the
    /// border — even with the 2-cell (4-dot) right inset.
    const CLOSE_TOP_INSET: u16 = 0;
