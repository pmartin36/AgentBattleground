/// Integration test for the `render_movement` example (b4-t1).
///
/// Shells out to `cargo run -p game --example render_movement -- --once` and
/// verifies the headless `--once` mode prints two distinct colored-braille
/// frames to stdout, separated by `---FRAME-BREAK---`.
///
/// The two frames must DIFFER despite the wizard moving less than one cell
/// between captures (sub-cell / dot-granularity movement proof).
///
/// Assertions:
///   (a) exits 0
///   (b) stdout splits on the marker into exactly 2 non-empty parts
///   (c) each part contains at least 1 braille glyph (U+2800..=U+28FF)
///   (d) the two parts are NOT identical — proves sub-cell dot shift changed the render
...
fn example_once_emits_two_distinct_braille_frames() {
    ...
    let output = Command::new(env!("CARGO"))
        ...
        .output()
        .expect("failed to spawn `cargo run -p game --example render_movement`");
    ...
    assert!(
        ...
        "render_movement --once must exit 0; got {:?}\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
        stdout,
        stderr
    );
    ...
    assert_eq!(
        parts.len(),
        2,
        "render_movement --once stdout must contain exactly one '{}' marker; \
         got {} part(s);\nstdout:\n{}",
        MARKER,
        parts.len(),
        stdout
    ...
    // (d) The two frames must differ — proves the sub-cell dot shift changed the render
    // (same wizard animation frame, different dot_x: 10 → 11).
    assert!(
        frame_a != frame_b,
        "frame A and frame B must differ (1-dot sub-cell shift must change the braille render);\
         \nframe A:\n{}\nframe B:\n{}",
        frame_a,
        frame_b
    );
    ...
    assert!(
        ...
        "render_movement --once stdout must contain an ANSI truecolor escape (\\x1b[38;2;);\
         \nstdout:\n{}",
        stdout
    );
}
