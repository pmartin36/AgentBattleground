/// Integration test for the `render_tier2` example (b3-t1).
///
/// Shells out to `cargo run -p game --example render_tier2 -- --once` and
...
///   (d) the two parts are NOT identical — proves the frame advanced
...
fn example_once_emits_two_distinct_braille_frames() {
    ...
    let output = Command::new(env!("CARGO"))
        ...
        .output()
        .expect("failed to spawn `cargo run -p game --example render_tier2`");
    ...
    assert!(
        ...
        "render_tier2 --once must exit 0; got {:?}\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
        stdout,
        stderr
    );
    ...
    assert_eq!(
        parts.len(),
        2,
        "render_tier2 --once stdout must contain exactly one '{}' marker; \
         got {} part(s);\nstdout:\n{}",
        MARKER,
        parts.len(),
        stdout
    ...
    // (d) The two frames must differ — proves the animation actually advanced.
    assert!(
        frame_a != frame_b,
        "frame A and frame B must differ (animation must have advanced by one frame_dur);\
         \nframe A:\n{}\nframe B:\n{}",
        frame_a,
        frame_b
    );
    ...
    assert!(
        ...
        "render_tier2 --once stdout must contain an ANSI truecolor escape (\\x1b[38;2;);\
         \nstdout:\n{}",
        stdout
    );
}
