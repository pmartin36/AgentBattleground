...
- **This applies to cross-cutting mechanisms (caching, batching, perf work), not just content.** If you're implementing something every scene/every game built on this engine would independently need to reinvent (e.g. "don't re-rasterize an unchanged sprite"), it belongs in `crates/engine/`, built so any caller — including a scene that doesn't know the mechanism exists — inherits it automatically. It does not belong as opt-in code repeated in each `crates/game/` scene. (Precedent: `32-static-asset-rasterization-caching` — caching lives in `engine-render`'s shared primitives, automatic for any caller, not something each scene wires up itself.)
