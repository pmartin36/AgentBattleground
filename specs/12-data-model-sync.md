...
- Opponent's 6 pieces: names, visuals, skill files, and the same stats/level/abilities/modifiers/stamina/squad-position fields the Army bullet above lists (`34-creature-attributes-data-model`) — the local sim needs the opponent's full piece data to run them, not just the player's own
