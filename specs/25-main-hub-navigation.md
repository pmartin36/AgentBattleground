...
- **Layout** (implementer's call within these constraints): title box top-center, the 3 menu buttons stacked below it, whole group roughly centered in the scene's render area. Exact spacing/sizing is not dictated here, same as Roster's per-slot layout was left to the pipeline.
