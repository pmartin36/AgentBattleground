...
> **Status: draft (not started).** Makes the game's text generation run on a real, bundled local model with no external apps required: a bundled llama.cpp runtime (sibling binary, like `sd-cli`), a registry of a few permissively-licensed models the player selects from, a download-on-select install with checksum verification, and a config that names a specific model. Turns spec `70`'s `Provider::Local` from a generic "run this command" into a registry-driven, installed model. Local is the shipped path; the online providers `70` built stay in the code as an advanced option, not the default.
...
- The config change: `Provider::Local` resolves a `model_id` through the registry to a concrete weights path + runtime invocation, replacing the generic free-form `local_command` as the normal path (the raw command stays as an escape hatch).
- The backend invocation change: build llama.cpp's real one-shot flags and pass the model weights + the framed prompt.

Out of scope: the model-selection/onboarding UI polish (`01`/`09` own the screens; this exposes the registry + install API they drive, plus a minimal selection mechanism); the persistent `llama-server` transport for the battle engine (a future addition for `10`, which makes many sequential calls); building/shipping the llama.cpp binaries per platform (a packaging step, like `sd-cli`'s — this spec's code assumes the sibling binary is present, as the asset pipeline already assumes `sd-cli`); the animation-weights licensing question (`needs-research/local-generation-service-integration.md`, a separate, unrelated H3 issue).
