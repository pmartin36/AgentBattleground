...
- **Consumes `34`'s data directly off `Creature`** (`stats()`/`level()`/`abilities()`/`exhaustion()` — no separate wrapper type) via `crate::creatures::demo_roster()` or its successor, the same way `RosterManager` already consumes `crate::creatures::all()` for the base `Creature` identity/animation data.
