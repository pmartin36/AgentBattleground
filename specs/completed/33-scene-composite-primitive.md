...
> Status: implemented. `composite_scene`/`SpriteContent`/`SpriteDraw` added to `crates/engine/render/src/composite.rs`; `BattleViewer::render()` migrated onto it. `piece_shape_and_color`, `piece_dots`, and `place_piece` deleted from `battle_viewer.rs` (confirmed zero remaining references workspace-wide). The 7 unaffected `piece_render_tests` tests pass unmodified; the 5 tests covering the deleted helpers had their coverage ported into `composite_scene`'s own test suite in `engine-render`. All existing `BattleViewer` render-output regression tests, and specs 27/29/32's regression suites, pass unmodified. Full workspace gate (`cargo test --workspace && cargo clippy --workspace --all-targets -- -D warnings`) is green.
