...
- **The event sequence is hand-authored/hardcoded directly in the scene** for this build, mirroring how the static `pieces()` layout is hardcoded (6v6 at the time of this build; the layout itself has since changed to 3v3 + bench per `36-battle-viewer-squad-layout`, but the "hardcode it, no file loading" reasoning this decision rests on is unaffected) — no file loading, no wire format.
