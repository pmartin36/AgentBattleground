...
> Status: implemented. `MainHub` (logo, title frame, cursor icon) and `RosterManager` (dot indicators) now decode bundled assets once at construction and read the stored `DynamicImage` in `render()`, mirroring `Button`/`FrameButton`'s existing pattern. Workspace-wide audit confirmed zero remaining per-frame `image::load_from_memory`/`draw_asset` call sites.
>
> Re-measured per this spec's own done-criterion: decode-only cost (~29ms) is eliminated and paid once at construction. Release-build `MainHub::render()` is ~2.6ms, a small fraction of the 33ms budget. Debug-build `MainHub::render()` still averages ~161ms — measurement showed the ~198ms baseline was never dominated by decode (~15-19% of it); the residual is per-frame Lanczos3 rasterization of the 1212×481 logo (`render::convert`/`sprite_to_dots`), which is rasterization-caching territory (`27-render-frame-caching`'s domain) for a static (non-`AnimatedSprite`) asset that spec doesn't cover. Per this spec's own explicit scope, debug-build performance is a separate process question, not resolved here; the regression test asserts the budget only in release and prints the debug number as documented evidence, not an assertion.
>
> Bundled raster assets (logos, panels, frames — anything drawn via `render::draw_asset` or similar ad-hoc `image::load_from_memory` calls) are decoded from raw PNG bytes fresh on *every single render call*, with no caching. Confirmed via direct timing in a debug build: `MainHub::render()` averages ~198ms/frame (vs. ~5ms/frame in release) — of which decoding the bundled logo alone costs ~29ms *per frame*, 30 times a second. This is very likely the root cause of user-reported "insane" button-click latency on the Main Hub menu (at ~200ms/frame, real framerate craters from the intended 30fps to ~5fps, so input can only be processed roughly every 200ms).
