...
> **Status: done.** Makes the game's LLM usage real and final: a bundled llama.cpp runtime (sibling binary, like `sd-cli`), a registry of a few permissively-licensed models with a default, a download-on-need install with checksum verification, a model-selection-centric config (a specific `model_id`, switchable, BYO later), and the mad-lib/creature-gen pipelines' hookup to it made real end-to-end. Turns spec `70`'s `Provider::Local` from a generic "run this command" into a registry-driven, installed model. Local is the shipped default; the online providers `70` built stay in the code as advanced / BYO options, not the promoted path.

## The LLM API (response shape)
`70`'s `generate_text` call keeps its name and its `text_gen` module (a rename to `respond` / `llm` was considered and dropped as a not-worth-it nit). The point is the shape, not the name: an LLM always returns text (tokens), and a prompt like "give me this mad-lib as JSON" returns a `String` that happens to be JSON-shaped, not a different type. The return stays `String` (the model's raw response); the caller parses it (labeled lines, JSON, an enum pick) and deterministically assembles. There is deliberately **no** `generate_json` and no operation returning a typed game entity — parts-not-entities (`70`) holds regardless of the response's format.
- For prompts that need reliable structure, the `generate_text` request MAY carry an optional output constraint (a GBNF grammar), applied natively by llama.cpp so the returned text conforms (useful for `parts.rs`'s `ARCHETYPE` pick; answers `70`'s constrained-decoding open question). The response is still a `String` the caller parses. Whether this ships in the first pass or is deferred is an open question below.
- `70`'s `TextBackend` trait, transport, conformance suite, routing, and cache are otherwise unchanged.
...
- An optional structured-output constraint (a GBNF grammar) on the existing `generate_text` request, for reliable structured responses (no call rename; see The LLM API above).
- The hookup: `67` / `71` call `generate_text` against the resolved local model, made real end-to-end (a mad-lib Done generates parts on the bundled local model).
