...
### Army Status Overview
A glanceable summary of your 6 pieces — names, visuals, and high-level health/readiness. "Readiness" now has a concrete meaning: whether a creature is available (active/bench) or injured and recovering (`34-creature-attributes-data-model`'s stamina/injury state) — this overview is a summary, not the full per-creature stamina/ability detail (that's the Roster screen, `35-roster-screen-stats-abilities-squad`).
...
## Dependencies
- `34-creature-attributes-data-model` — the stamina/injury state the Army Status Overview summarizes.
