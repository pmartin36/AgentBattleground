...
## Decisions (v1)
- **Lives in `crates/game`.** `Creature` (name + `AnimationKind` → `AnimatedSprite` catalog) is `crates/game/src/creatures.rs`'s own type — `engine_render` retains only the generic `AnimatedSprite` primitive. Stats/abilities/modifiers/exhaustion/squad-role are Agent-Battleground-specific RPG mechanics with no meaning to a different game, so per the engine/game boundary rule they belong in `crates/game`. Concretely: a new type in `crates/game/src/creatures.rs` (e.g. `RosterEntry`) pairs a `Creature` (identity/animation) with the new game-specific fields — the same shape of split that already exists between `Piece` (game-specific battle state, `crates/game/src/scenes/battle_viewer.rs`) and the engine's generic sprite/transform types.
