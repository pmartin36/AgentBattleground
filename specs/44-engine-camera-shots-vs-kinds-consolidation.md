...
- Any new camera capability, preset, or shot. This is a pure consolidation: behavior must be pixel-identical to today for Top-down (unaffected) and Sideline/Over-the-shoulder/Free-roam (re-expressed, not changed) — verified via the existing golden-fixture and property tests, not by asserting the new constants are correct by construction.
...
```
`cam_space`, `cam_forward_raw`, `forward_distance`, `half_fov_tan`, `project`, `depth_key`, `vertical_anchor_hint` (`Bottom`), `elevation_deg()` (returns `pitch_deg`), `local_dots_per_world_unit`, and `nudge()` all move here verbatim from `FreeRoamCamera` (`crates/engine/render/src/camera/free_roam.rs`), replacing `PerspectiveCamera`'s current `DepthAxis`-based versions. `facing_sign` is deleted entirely — as `FreeRoamCamera`'s own doc comment already states, `yaw_deg` alone determines facing via `cam_space`, so the field that existed only to compensate for `DepthAxis` not encoding a real direction (and whose whole doc comment describes exactly that compensation) is no longer needed. `depth_axis`, `camera_depth`, `spread_center`, `camera_height` are deleted (the last renamed to `height`, absorbed as-is). `DepthAxis` and `axis_values` (`camera/mod.rs`) are deleted — confirmed dead: `.depth_axis` is referenced nowhere outside `perspective.rs` itself.
...
- `sideline_preset()`: `AnyCamera::Perspective(PerspectiveCamera { x: SIDELINE_CAMERA_DEPTH, y: BOARD_CENTER_COL, height: 2.5, yaw_deg: 90.0, pitch_deg: 10.0, fov_deg: 55.0, scale_dots: 0.0 })`, `fit: FitMode::ViewportFit`. (This exact mapping — `yaw_deg: 90.0` for Sideline's prior `depth_axis: Col, facing_sign: 1.0` — is already validated: it's exactly what spec 42's `free_roam_preset()` used as its Sideline-derived starting transform, confirmed correct by that spec's own manual verification pass.)
- `over_shoulder_preset()`: `AnyCamera::Perspective(PerspectiveCamera { x: BOARD_CENTER_COL, y: OVER_SHOULDER_CAMERA_DEPTH, height: OVER_SHOULDER_CAMERA_HEIGHT, yaw_deg: 180.0, pitch_deg: 30.0, fov_deg: 55.0, scale_dots: 0.0 })`, `fit: FitMode::ViewportFit`. **This specific mapping is a worked derivation, not a pinned/verified constant** — `yaw_deg: 180.0` is derived from `over_shoulder_preset`'s old `depth_axis: Row` (depth=world-y, spread=world-x) + `facing_sign: -1.0` (looks toward decreasing row) via `FreeRoamCamera`'s convention (`yaw=0` faces `+y`, so facing `-y` is `yaw=180`), and `x`/`y` from swapping `spread_center`/`camera_depth` onto world-x/world-y per the Row-axis assignment. The implementing task must verify this reproduces today's actual behavior against the existing property tests (`overshoulder_project_and_depth_key_match_wrapped_perspective` and friends) and golden fixtures before treating it as correct — a sign error here (e.g. `yaw_deg: 0.0` instead of `180.0`, or `x`/`y` swapped) would still compile and still be a valid camera, just aimed wrong, so compilation success proves nothing here.
...
## Open Questions / TBDs
None blocking. The one item flagged as needing verification rather than trust — Over-the-shoulder's derived `x`/`y`/`yaw_deg` mapping (Decision 3) — is not a design ambiguity, it's an arithmetic translation that must be checked against this codebase's own existing property tests and golden fixtures as part of implementation, exactly the kind of thing this project's "verification is grounded in observed truth, never assumption" rule exists for.
